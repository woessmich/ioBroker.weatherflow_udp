...
class WeatherflowUdp extends utils.Adapter {
  ...
  async myCreateState(
    ...
    if (!existingStates.includes(stateName)) {
      ...
      const obj = await this.setObjectNotExistsAsync(
        stateName,
        stateParameters,
      ); // create if not existing and log creation
      if (obj) {
  ...
  async calcMinMaxValue(stateName, stateParameters, stateValue, calcType) {
    ...
    const minmaxStateParametersToday = JSON.parse(
      JSON.stringify(stateParameters),
    ); // Make a real copy not just an addtl. reference
    const minmaxStateParametersYesterday = JSON.parse(
      JSON.stringify(stateParameters),
    ); // Take parameters from main value ...
    ...
    switch (calcType) {
      case MIN:
        ...
        minmaxStateParametersToday.common.name +=
          " / today / min; adapter calculated"; // ... and add something to the name
        minmaxStateNameYesterday = `${stateBase}.yesterday.min.${state}`; // create state name
        minmaxStateParametersYesterday.common.name +=
          " / min / yesterday; adapter calculated"; // ... and add something to the name
        break;

      case MAX:
        minmaxStateNameToday = `${stateBase}.today.max.${state}`; // create state name
        minmaxStateParametersToday.common.name +=
          " / today / max; adapter calculated"; // ... and add something to the name
        minmaxStateNameYesterday = `${stateBase}.yesterday.max.${state}`; // create state name
        minmaxStateParametersYesterday.common.name +=
          " / yesterday / max; adapter calculated"; // ... and add something to the name
        break;
    ...
    if (obj !== null) {
      if (now.getDay() === oldNow.getDay()) {
        ...
        if (obj.val !== newMinmaxValue) {
          // only update state if value is different
          this.myCreateState(
            minmaxStateNameToday,
            minmaxStateParametersToday,
            newMinmaxValue,
          ); // create and/or write node
        }
      } else {
        // new day, always update
        this.myCreateState(
          minmaxStateNameToday,
          minmaxStateParametersToday,
          stateValue,
        ); // On a new day, first value is the minimum of 'today'
        this.myCreateState(
          minmaxStateNameYesterday,
          minmaxStateParametersYesterday,
          obj.val,
        ); // Values for yesterday are last min value from today
      }
    } else {
      // min or max state does not yet exist, create
      this.myCreateState(
        minmaxStateNameToday,
        minmaxStateParametersToday,
        stateValue,
...
function getQFF(temperature, airPressureAbsolute, altitude, humidity) {
  ...
  const e_d =
    f_rel * E_0 * Math.exp((17.5043 * temperature) / (241.2 + temperature));
  ...
  const reducedPressure =
    Math.round(
      10 *
        ...
        Math.exp(
          (g_n * altitude) /
            (R * (temperature + T_0 + C * e_d + (gam * altitude) / 2)),
        ),
    ) / 10;
...
function dewpoint(temperature, humidity, pressure = undefined) {
  ...
  const absoluteHumidity =
    (((Math.pow(10, 5) * mw) / gk) * DD) / (temperature + t0);
...
function feelsLike(temperature, windspeed, humidity) {
  ...
  } else if (temperature < 10 && windspeed > 5 / 3.6) {
    // wind chill (https://de.wikipedia.org/wiki/Windchill)
    feelsLikeTemperature =
      13.12 +
      0.6215 * temperature +
      (0.3965 * temperature - 11.37) * (windspeed * 3.6) ** 0.16;
  } else {
